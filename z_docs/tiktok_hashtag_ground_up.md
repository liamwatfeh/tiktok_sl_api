...
## 🎯 **API Overview**
- **First Endpoint to build ya**: `/analyze-tiktok-hashtags`
...
**💡 NOTE**: When testing this API client, we should examine actual response schemas to inform data cleaning logic in Phase 4.
...
**⚠️ IMPORTANT - BEFORE IMPLEMENTING**: Request actual API response examples from user for:
1. **Hashtag endpoint response**: `GET /challenge/{hashtag}/feed` 
2. **Comments endpoint response**: `GET /comments/{video_id}`

Different endpoints may return different JSON schemas, requiring endpoint-specific cleaning logic.
    ...
    def _collect_hashtag_videos(self, hashtag: str, count: int) -> Tuple[List[Dict], int]:
        ...
        while len(videos) < count and api_calls < max_attempts:
            try:
                ...
                # Make API call
                logger.debug(f"Calling Challenge Feed API: hashtag={hashtag}, cursor={cursor}")
                response = self.api_client.challenge_feed(
...
class TestTikTokHashtagEndpoint:
    ...
    async def test_complete_hashtag_analysis_pipeline(
        ...
        with patch('app.services.tiktok_shared.tiktok_api_client.TikTokAPIClient') as mock_client_class:
            ...
            # Mock API responses
            mock_client.challenge_feed.return_value = mock_tiktok_challenge_feed_response
